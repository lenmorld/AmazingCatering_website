   ...
   //prepare update query, time is not included
    ...
    //        SET message = ?, date = ? 
     ...
     if (!empty($_POST['message']))
     {
        ...
        $nameSender = $_SESSION['username'];
     
        $sql = "INSERT INTO messages
			(reserve_ID,date_added,message,sender)
			VALUES (" . "$reserve_ID, '$curDateMysql', '$msg', '$nameSender')";
...
if (isset($_POST['uploadA']))
{
	...
	$dest = './uploads/attach/' ;			// destination of uploaded files
...
if ($done  || !isset($_GET['reserve_ID'])) { 
  ...
  header('Location: ' . "./reserve_message.php?reserve_ID=$reserve_ID"); 	//redirect to list of reservations
    ...
    $sql = "SELECT filename
      ...
      if ($numRows2)
      {
        ...
        while($row2 = $resultDB2->fetch_assoc())
        {
          ...
          echo "<br />";
          $file3 =  $row2['filename'];
          echo '<a target="_blank" href="./uploads/attach/' . $file3 . '">' . $file3  . '</a>';
        }
        echo "</div>";
      }
    ...
    <div style="border: 5px red inset; padding: 10px;">
        ...
        <form method="post" action="" name="postmsg">
            ...
            <?php
                ...
                <a target="_blank" href="./uploads/attach/<?php echo  $file ;?> ">PREVIEW FILE</a><br/>
                <?php
